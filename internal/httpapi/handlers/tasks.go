...
// Она используется для подготовки данных к отправке в формате JSON в ответ на запрос создания новой задачи.
// Функция принимает объект domain.Task и возвращает объект gen.CreateTaskResponse, который содержит поля Id, Task и IsDone, соответствующие полям из domain.Task.
...
// Она используется для подготовки данных к отправке в формате JSON в ответ на запрос обновления существующей задачи.
// Функция принимает объект domain.Task и возвращает объект gen.PatchTaskResponse, который содержит поля Id, Task и IsDone, соответствующие полям из domain.Task.
...
// toListTasksResponse - это функция, которая преобразует срез структур domain.Task в срез структур gen.Task, который является типом gen.ListTasksResponse.
// Она используется для подготовки данных к отправке в формате JSON в ответ на запрос получения списка задач.
// Функция принимает срез объектов domain.Task и возвращает срез объектов gen.Task, которые содержат поля Id, Task и IsDone, соответствующие полям из domain.Task.
