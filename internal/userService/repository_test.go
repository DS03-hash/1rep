...
func TestGormRepository_GetTasksForUser(t *testing.T) {
	t.Parallel()
	...
	t.Run("success", func(t *testing.T) {
		...
		rows := sqlmock.NewRows([]string{"id", "task", "is_done", "user_id", "deleted_at"}).
			AddRow(11, "task 1", false, 7, nil).
			AddRow(12, "task 2", true, 7, nil)

		mock.ExpectQuery(`SELECT .* FROM "tasks" WHERE user_id = \$1 AND "tasks"\."deleted_at" IS NULL`).
			WithArgs(7).
			WillReturnRows(rows)

		repo := NewGormRepository(db)
		got, err := repo.GetTasksForUser(7)

		require.NoError(t, err)
		require.Len(t, got, 2)
		assert.Equal(t, domain.Task{ID: 11, Task: "task 1", IsDone: false, UserID: 7}, got[0])
		assert.Equal(t, domain.Task{ID: 12, Task: "task 2", IsDone: true, UserID: 7}, got[1])
	})
	...
	t.Run("query error", func(t *testing.T) {
		...
		repo := NewGormRepository(db)
		got, err := repo.GetTasksForUser(9)
...
// TestGormRepository_Update проверяет обновление пользователя в репозитории.
...
// TestGormRepository_DeleteByID проверяет удаление пользователя по идентификатору.
